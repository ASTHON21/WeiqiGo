 ...
 * 指挥层 (ShadowEngine)
 * 职责：决策总调度。
 * 优先级：1. SGF 字典匹配 -> 2. Alpha-Beta 启发式搜索
 */
    ...
    public findBestMove(
        ...
        if (sgfMatch) {
            return {
                ...
                explanation: `[本能响应] 匹配到定式库建议：执行《${sgfMatch.source}》中的高手走法。`,
                gamePhase: 'Joseki'
            };
        }
        ...
        if (possibleMoves.length === 0) {
            return { bestMove: null, explanation: "棋局结束。", gamePhase: currentPhase };
        }
        ...
        return {
            bestMove: bestMove,
            explanation: `[启发式搜索] 基于 Alpha-Beta 算法，预估选点分值：${bestValue.toFixed(1)}`,
    ...
    private alphaBeta(
        ...
        if (isMaximizing) {
            ...
            for (const move of moves) {
                ...
                if (!result.success) continue;
                const evaluation = this.alphaBeta(result.newBoard, depth - 1, alpha, beta, false, aiPlayer, moveCount + 1, [...boardHistory, result.newBoard]);
            ...
            for (const move of moves) {
                const result = GoLogic.processMove(board, move.r, move.c, currentPlayer, boardHistory);
                if (!result.success) continue;
                const evaluation = this.alphaBeta(result.newBoard, depth - 1, alpha, beta, true, aiPlayer, moveCount + 1, [...boardHistory, result.newBoard]);
